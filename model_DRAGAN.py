...
if __name__ == '__main__':
    ...
    train_loader = torch.utils.data.DataLoader(
        mixed, batch_size=batch_size, shuffle=True, drop_last=True)
        ...
        for batch_idx, data in enumerate(train_loader):
            ...
            if batch_idx % 100 == 0:
                vutils.save_image(data,
                        '/progress_check/pics/real_samples.png')
                fake = generator(z)
                vutils.save_image(gen.data.view(batch_size, 1, 64, 64),
                        '/progress_check/pics/fake_samples_epoch_%03d.png' % epoch)
    
    #save checkpoint afterwards
    torch.save(generator.state_dict(), f'./savepoint/after_hf_G.pth')
    torch.save(discriminator.state_dict(), f'./savepoint/after_hf_D.pth')
